...
MUTATIONS = [
    ...
    ("outcome-without-offer", "agentdm/store.py", 'if not offer or offer["alias"] != alias:\n            raise AgentdmError("message was never offered to that alias")\n        if offer["offered_to"] != (inc or HUMAN):\n            raise AgentdmError("message was offered to a different incarnation; fetch it first")\n        if os.path.exists(self._p("outcomes"',
     ...
     "test_intents.IntentAcceptance.test_in02_outcomes_need_an_offered_request_kind"),
    ("envelope-unavailable-as-zero", "agentdm/awareness.py", 'return "unavailable"', 'return {"unread": 0, "pending_requests": 0}',
     "test_intents.AwarenessAcceptance.test_in07_unreadable_mailbox_reads_unavailable_never_zero"),
    ("claims-sentence-unearned", "agentdm/awareness.py", 'if env["pending_requests"] and holds_claim(store, inc):', 'if True:',
     "test_intents.AwarenessAcceptance.test_in06_every_response_carries_counts_and_the_claims_sentence_only_when_earned"),
    ("wait-diagnosis-dropped", "agentdm/server.py", '**self._diagnosis(from_alias, message_id)}', '**{}}',
     "test_intents.AwarenessAcceptance.test_in08_wait_timeout_diagnoses_the_peer_and_the_request"),
    ("reply-without-receipt", "agentdm/store.py", 'outcome_for=(message_id, outcome))', 'outcome_for=None)',
    ...
    ("unknown-claim-accepted", "agentdm/store.py", 'if about_claim is not None and not _read_json(self._p("claims", f"{_mid_key(str(about_claim))}.json")):', 'if False:',
     "test_intents.ContestedClaimAcceptance.test_in10_a_request_about_a_claim_marks_it_contested_until_answered"),
    ("glance-creates-store", "agentdm/cli.py", 'store = find_store(os.environ.get("AGENTDM_PROJECT_DIR") or os.getcwd())', 'store = resolve_store(os.environ.get("AGENTDM_PROJECT_DIR") or os.getcwd())',
     "test_cli_contract.GlanceAcceptance.test_cl05_glance_prints_counts_only_and_creates_nothing"),
    ("push-without-ids", "agentdm/notify.py", 'if event.get("message_id"):', 'if False:',
     "test_notify.NotifyIdsAcceptance.test_nt07_pushes_carry_ids_never_paths"),
